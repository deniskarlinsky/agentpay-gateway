...
public class OutboxPublisher {
  ...
  public OutboxPublisher(
      EventOutboxRepository outbox, KafkaTemplate<String, byte[]> kafkaTemplate, Clock clock) {
    this.outbox = outbox;
    this.kafkaTemplate = kafkaTemplate;
    this.clock = clock;
  }

  // TODO (Iter 6, observability): tune OutboxPublisher fixedDelay; current 200ms is the
  // worst-case state-event-to-Kafka latency observable to consumers. Acceptable for now
  // since no realtime dashboard exists yet.
  @Scheduled(fixedDelayString = "${agentpay.outbox.poll-interval-ms}")
